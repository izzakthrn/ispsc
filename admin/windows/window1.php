...
<body>
    <div class="window-container">
        ...
        <div class="window-body">
            ...
            <!-- Currently Serving -->
            <div class="section-label" id="serving-label">
    ...
    <script>
        ...
        function updateUI(data) {
            ...
            // Update serving count in header
            const servingCount = data.current_serving ? data.current_serving.length : 0;
            ...
            servingLabel.textContent = `Serving ${servingCount} Customer${servingCount !== 1 ? 's' : ''}`;

            // Update serving section
            const servingSection = document.getElementById('serving-section');
            if(data.current_serving && data.current_serving.length > 0) {
                servingSection.innerHTML = data.current_serving.map(serving => `
                    <div class="serving-card">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <span class="serving-badge">Now Serving</span>
                                <h2 class="queue-number">${escapeHtml(serving.queue_number)}</h2>
                                <p class="customer-name">${escapeHtml(serving.customer_name)}</p>
                                <p class="service-label">${escapeHtml(serving.service_name)}</p>
                            </div>
                            <button class="btn btn-done" onclick="markDone(${serving.id})">
                                <i class="bi bi-check-circle"></i>
                                Done
